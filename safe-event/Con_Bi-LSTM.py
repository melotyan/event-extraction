...
MODEL_SAVE_PATH = 'con-bilstm-trigger2.h5'
num_lstm = 100
...
#数据获取
words, train_x, train_local, train_y = init_format_data('word-format.txt')
#转成category格式
from keras.utils.np_utils import to_categorical
train_y = to_categorical(train_y, num_classes=2)
print ('总样本数', sum(np.argmax(a) for a in train_y))
# train_y = np.asarray(train_y)
test_x, test_local, test_y = train_x[-600:], train_local[-600:], train_y[-600:]
train_x, train_local, train_y = train_x[:-600], train_local[:-600], train_y[:-600]
...
cnn_layer2 = Conv1D(filters=32, kernel_size=(5))
cnn_layer3 = Conv1D(filters=32, kernel_size=(7))
#构建模型
...
x2 = cnn_layer1(lexical_embedding_sequence)
x3 = cnn_layer2(lexical_embedding_sequence)
x4 = cnn_layer3(lexical_embedding_sequence)
...
model.fit({'sentence_input':train_data, 'lexical_input':train_local_feature}, {'output':train_label}, validation_data=([val_data, val_local], val_label), validation_split = 0.1, epochs=50, batch_size=BATCH_SIZE,
          shuffle=True, callbacks=[early_stopping, check_point], class_weight={1:8, 0:1})#,
# model.save('con-bilstm.h5')
model.load_weights(best_weigth_path)

#测试
y_pred = model.predict([test_data, test_local_feature], batch_size=300, verbose=1)
