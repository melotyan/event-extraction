...
from init_training_data import init_char_safe_data, init_word_safe_data, init_test_word_safe_data
...
BATCH_SIZE = 32
WORD_TO_VECTOR_PATH = '../word-vec100.txt'
MODEL_SAVE_PATH = 'embedding-trigger.h5'


#获取数据
train_x, train_y = init_word_safe_data(path='safe-event.txt', max_len=MAX_LEN)
train_y = np.asarray(train_y)
test_x, test_y = train_x[-70:], train_y[-70:]
train_x, train_y = train_x[:-70], train_y[:-70]
print ('train_y shape', train_y.shape, train_x[1], train_y[1])
...
#临时测试用
# test_x1, test_y1 = init_test_word_safe_data(path1='safe-event.txt', path2 = 'safe-event2.txt', max_len=MAX_LEN)
# test_x = np.concatenate([test_x, test_x1])
# test_y = np.concatenate([test_y, test_y1])
...
print ('precision count', true_positive, 'count is', pred_positive, 'target count is', actual_positive)
print ('precision', true_positive * 1.0 / pred_positive, 'recall', true_positive * 1.0 / actual_positive)
