...
MODEL_SAVE_PATH = 'multi-convolution2.h5'
num_lstm = 100
TEST_NUM = 10000
...
#数据获取
words, train_x, train_lexical, train_pos, train_y = get_multi_conv_data('word-multi-conv-big.json')
...
test_word, test_x, test_local, test_pos, test_y = words[-TEST_NUM:], train_x[-TEST_NUM:], train_lexical[-TEST_NUM:], train_pos[-TEST_NUM:], train_y[-TEST_NUM:]
train_x, train_lexical, train_pos, train_y = train_x[:-TEST_NUM], train_lexical[:-TEST_NUM], train_pos[:-TEST_NUM], train_y[:-TEST_NUM]
print ('train shape', np.asarray(train_x).shape, train_pos.shape, train_y.shape)
print ('训练样本数', sum(np.argmax(a) for a in train_y), '测试样本数', sum(np.argmax(a) for a in test_y))
