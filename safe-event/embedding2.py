...
from init_training_data import init_char_safe_data, init_word_safe_data
...
BATCH_SIZE = 32
WORD_TO_VECTOR_PATH = '../char-vec400.txt'
MODEL_SAVE_PATH = 'embedding-trigger.h5'


#获取数据
train_x, train_y = init_char_safe_data(path='safe-event.txt', max_len=MAX_LEN)
...
print ('train_y shape', train_y.shape, train_y[0])
...
for i in range(len(y_pred)):
    ...
    pred_temp = y_pred[i].reshape(MAX_LEN, 3)
    true_temp = test_y[i].reshape(MAX_LEN, 3)
    ...
    for k in range(min(len(test_arr), len(pred_temp))): #判断每一个字符
        ...
        if true_max_indexs[0] == 1:
            ...
            target_count += 1
            print (test_arr[k], end='')
        elif true_max_indexs[0] == 2:
            print (test_arr[k], end='')

    print()
    # print (pred_temp)
    # print (true_temp)
    # print ('测试预测', pred_max_indexs)
    # print ('测试真实', true_max_indexs)
    #
