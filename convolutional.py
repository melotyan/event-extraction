...
MODEL_SAVE_PATH = 'eng-event/convolutional-trigger.h5'




#数据获取
train_x, train_y = init_eng_wiki_trigger_data(max_len=MAX_SEQUENCE_LENGTH)
...
cnn_layer = Conv1D(filters=200, kernel_size=(3), kernel_initializer=RandomUniform(minval=-0.01, maxval=0.01, seed=729230112))

#构建模型
inp = Input(shape=(MAX_SEQUENCE_LENGTH,), dtype='int32')
embedding_sequence = embedding_layer(inp)
x1 = Flatten()(embedding_sequence)
...
model.compile(optimizer='adadelta', loss='binary_crossentropy', metrics=[auc])
model.summary()
