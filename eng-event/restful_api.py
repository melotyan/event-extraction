...
def build_netword(nb_words, embeddings_matrix):
    '''
    建网络模型
    :param nb_words:
...
def train(model, weight_path, train_data, train_label, val_data, val_label):
    earlystoping = EarlyStopping(monitor='val_loss', patience=5)
    # best_weight_path = MODEL_SAVE_PATH
    model_chekpoint = ModelCheckpoint(weight_path, save_best_only=True, save_weights_only=True)
...
def predict(tokenizer, sentences):
    ...
    y_pred = model.predict(data, batch_size=30, verbose=1)
    y_pred[y_pred >= 0.5] = 1
    y_pred[y_pred < 0.5] = 0
...
def evaluate(model, test_data, test_x, test_y):
    #测试
...
def load_my_model():
    ...
    model = build_netword(nb_words=nb_words, embeddings_matrix=embedding_matrix)
...
if __name__ == '__main__':
    ...
    # predict(model, tokenizer, ['Edward II was the fourth son of Edward I and his first wife, Eleanor of Castile.',
    #                  'Natalie is the winner of Miss Universe 2005 and also the wife of Thai tennis player Paradorn Srichaphan.',
