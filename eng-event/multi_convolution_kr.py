...
def get_data():
    '''
    get training data and split them to train and test
    :return:
    '''
    #load training data
    ...
    return word_index, train_data, train_lexical_feature, train_pos, train_y, test_data, test_local_feature, test_pos, test_y

def get_word_embedding(word_index):
    #read pre-trained word vectors
    embeddings_index = {}
    with open(WORD_TO_VECTOR_PATH) as f:
        #first line is not vector
        f.readline()
        for line in f:
            values = line.split()
            word = values[0]
    ...
    #extract word-embedding matrix 提取需要的词向量矩阵
...
def build_network(nb_words, embeddings_matrix):
    #定义网络层
...
def evaluate(model, test_data, test_local_feature, test_pos, test_y):
...
if __name__ == '__main__':
    word_index, train_sen_feature, train_lexical_feature, train_pos_feature, train_y, \
    test_sen_feature, test_lexical_feature, test_pos_feature, test_y = get_data()
    nb_words, embeddings_matrix = get_word_embedding(word_index)
    model = build_network(nb_words, embeddings_matrix)
    train(model, train_sen_feature, train_lexical_feature, train_pos_feature, train_y)
    evaluate(model, test_sen_feature, test_lexical_feature, test_pos_feature, test_y)
