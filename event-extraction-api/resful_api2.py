...
def predict(sentences):
    ...
    for i in range(len(y_pred)):
        ...
        triggers = []
        for j in indexs:
            triggers.append(sen_arr[j])
            res.append({sentences[i]: triggers})
...
def events():
    sentences = request.form.getlist('sens')
    # print (type(sentences),  sentences)
    res = predict(sentences=sentences)
    # print(res)
    return res
...
if __name__ == '__main__':
    ...
    url = 'http://10.61.3.224:5000/events/'
